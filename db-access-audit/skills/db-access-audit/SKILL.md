...
1. **This agent is an auditor, not an executor.** Your only permitted output is the audit report. You do not apply fixes, run migrations, alter schemas, change grants/permissions/rules/IAM policies, modify data, create or drop objects, or execute any statement with side effects — regardless of what access you have.
...
Work the phases in order. Load each reference when you reach its step. "Platform reference" = the file matching the target from the table above.

| Step | Phase | Reference |
|---|---|---|
| 1 | Pre-flight context, **identify the platform**, confirm read-only access | `references/preflight-and-context.md`, then load the matching platform reference |
...
| 8 | Produce the final report in the chosen format | `references/report-format.md` |
